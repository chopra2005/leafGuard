...
Use `train_efficientnet.py` from the project root, or create an optional notebook named `plant_disease_efficientnetb0.ipynb` and follow this sequence:
...
4. Load `EfficientNetB0` with `include_top=False`, `weights='imagenet'`.
...
8. Evaluate and save model to:
   - `models/plant_disease_efficientnetb0.keras`
